...
export default function ProductDetailTemplate({ product }) {
  if (!product) {
    return (
      <div style={{ padding: "80px 20px", textAlign: "center" }}>
        ...
        <Link href="/products" className="primary-btn1 white-bg">
  ...
  const whatsappNum = (product.whatsappNumber || "917096007670").replace(/[^0-9]/g, "");
  ...
  return (
    <>
            ...
            >
              <div className="about-img">
                <img
                  alt={product.title}
                  className="w-100"
                  src={product.image || "/assets/img/product/product1.jpg"}
                />
              </div>
            </div>
              ...
              <div className="about-full-box-page-left">
                <div className="about-content">
                  {product.paragraphs && product.paragraphs.length > 0 ? (
                    product.paragraphs.map((p, idx) => <p key={idx}>{p}</p>)
                  ) : (
                    <p>
                      The <span className="red fw-semibold pr-1">ANJANI</span>{" "}
                      {product.title} is designed for efficient fabric dyeing with
                      low liquor ratio, reliable process performance, and optimized power and water consumption.
                    </p>
                  )}

                  {product.capacity && <h5>CAPACITY: {product.capacity}</h5>}
                </div>

                <div className="btb-pd-details">
                  <h5>Request Technical Offer</h5>
                  <div className="btb-pd-btn">
                    <a
                      className="primary-btn1 whatsapp-btn"
                    ...
                    {product.brochureUrl && (
                      <a
                        className="primary-btn1 download-brochure-btn"
            ...
            <div className="row">
              <div className="col-lg-12">
                <div className="section-title text-center mb-4">
                  <h2 className="mb-2">Key Advantages</h2>
                </div>
              </div>
            </div>
            <div className="row">
              {product.keyAdvantages.map((item, idx) => (
      ...
      {/* SECTION 4 - SPACE REQUIREMENT MODEL */}
      {product.spaceModel && product.spaceModel.length > 0 && (
        <div className="pd-table mb-80">
          <div className="container">
            <div className="row">
              <div className="col-lg-12">
                <div className="section-title text-center mb-4">
                  <h2 className="mb-2">Space Requirement Model</h2>
                </div>
              </div>
              <div className="col-lg-12">
                <div className="space-model__table-wrap">
                  <table className="space-model__table">
                    <thead>
                      <tr>
                        <th>MODEL</th>
                        {product.spaceModel[0].capacity !== undefined && <th>CAPACITY</th>}
                        {product.spaceModel[0].width !== undefined && <th>WIDTH</th>}
                        <th>LENGTH</th>
                        {product.spaceModel[0].breadth !== undefined && <th>BREADTH</th>}
                        <th>HEIGHT</th>
                      </tr>
                    </thead>
                    <tbody>
                      {product.spaceModel.map((row, idx) => (
                        <tr key={idx}>
                          <td>{row.model}</td>
                          {row.capacity !== undefined && <td>{row.capacity}</td>}
                          {row.width !== undefined && <td>{row.width}</td>}
                          <td>{row.length}</td>
                          {row.breadth !== undefined && <td>{row.breadth}</td>}
                          <td>{row.height}</td>
                        </tr>
                      ))}
                    </tbody>
